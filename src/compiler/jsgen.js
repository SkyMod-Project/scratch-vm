...
class JSGenerator {
    ...
    descendInput (node) {
        ...
        case 'op.ternary':
            return new TypedInput(`(${this.descendInput(node.operand).asBoolean()} ? ${this.descendInput(node.left).asString()} : ${this.descendInput(node.right).asString()})`, TYPE_STRING);
        case 'op.length':
        ...
        case 'op.average':
            // Needs to be marked as NaN because (Infinity + -Infinity) / 2 = Nan
            return new TypedInput(`((${this.descendInput(node.left).asNumber()} + ${this.descendInput(node.right).asNumber()}) / 2)`, TYPE_NUMBER_NAN);
        case 'op.round':
    ...
    descendStackedBlock (node) {
        ...
        case 'motion.setDirection':
            ...
            break;
        case 'motion.pointXY': {
            const targetX = this.descendInput(node.x).asNumber();
            const targetY = this.descendInput(node.y).asNumber();

            const dx = targetX - this.target.x;
            const dy = targetY - this.target.y;
            const dir = 90 - MathUtil.radToDeg(Math.atan2(dy, dx));
            this.source += `target.setDirection(${dir});`;

            break;
        }
        case 'motion.pointInRandomDirection':
            this.source += 'target.setDirection(Math.round(Math.random() * 360) - 180);\n';
            break;
        ...
        case 'motion.changeXY': {
            ...
            const x = 'x' in node ? this.descendInput(node.x).asNumber() : 'target.x';
            const y = 'y' in node ? this.descendInput(node.y).asNumber() : 'target.y';
            this.source += `target.setXY(target.x + ${x}, target.y + ${y});\n`;
            if (this.descendedIntoModulo) {
                this.source += `if (target.interpolationData) target.interpolationData = null;\n`;
            }
            break;
        ...
        case 'leftAroundXY': {
            const degrees = this.descendInput(node.degrees).asNumber();

            const center = {
                x: this.descendInput(node.x).asNumber(),
                y: this.descendInput(node.y).asNumber()
            };
            ...
            this.source += `target.setXY(${newPosition.x}, ${newPosition.y})`;
